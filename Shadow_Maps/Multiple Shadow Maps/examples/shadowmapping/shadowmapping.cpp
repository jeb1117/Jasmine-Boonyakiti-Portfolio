...
public:
	...
	struct {
		...
		VkPipeline sceneShadowVariance;
	} pipelines;
	...
	~VulkanExample()
	{
		...
		vkDestroyPipeline(device, pipelines.sceneShadowVariance, nullptr);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			if (filterVarianceMap)
			{
				...
				vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, (filterVarianceMap) ? pipelines.sceneShadowVariance : pipelines.sceneShadow);
...
#undef QUAD_COLOR_NORMAL
	...
	void preparePipelines()
	{
		...
		uint32_t enableVariance = 0;
		VkSpecializationMapEntry newSpecializationMapEntry = vks::initializers::specializationMapEntry(0, 0, sizeof(uint32_t));
		VkSpecializationInfo newSpecializationInfo = vks::initializers::specializationInfo(1, &newSpecializationMapEntry, sizeof(uint32_t), &enableVariance);
		shaderStages[1].pSpecializationInfo = &newSpecializationInfo;

		enableVariance = 1;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.sceneShadowVariance));
	...
	virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
	{
		if (overlay->header("Settings")) {
			...
			if (overlay->checkBox("Variance filtering", &filterVarianceMap))
			{
				buildCommandBuffers();
			}
		}
